...
namespace FirnessTracker.API.Tests.WorkoutControllerTests
{
    ...
    {
        [Fact]
        public async Task GetById_ShouldThrowEntityNotFoundException_WhenWorkoutNotExists()
        {
        ...
        [Fact]
        public async Task GetById_ShouldThrowAccessDeniedException_WhenUserIsNotOwner()
        {
            var workoutsRepositoryMock = new Mock<IWorkoutsRepository>();
            var authorizationServiceMock = new Mock<IAuthorizationService>();
            var mapperMock = new Mock<IMapper>();
